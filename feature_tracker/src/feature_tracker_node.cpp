...
void img_callback(const sensor_msgs::msg::Image::ConstSharedPtr img_msg) {
    ...
    // detect unstable camera stream 이미지 연속 입력 확인
    if (img_msg->header.stamp.sec + img_msg->header.stamp.nanosec * (1e-9) - last_image_time > 1.0 || img_msg->header.stamp.sec + img_msg->header.stamp.nanosec * (1e-9) < last_image_time) {
        ...
        pub_restart->publish(restart_flag); // 중단 발행
        return;
    }

    last_image_time = img_msg->header.stamp.sec+img_msg->header.stamp.nanosec * (1e-9);
    // frequency control -> Publish 주기 조절
    ...
    if (PUB_THIS_FRAME) {
        ...
        auto feature_points = std::make_unique<sensor_msgs::msg::PointCloud>();
        ...
        for (int i = 0; i < NUM_OF_CAM; i++) {
            ...
            for (unsigned int j = 0; j < ids.size(); j++) {
                if (trackerData[i].track_cnt[j] > 1) {
                    ...
                    p.x = un_pts[j].x;  // 왜곡제거된 피쳐담아서 넘겨준다
                    p.y = un_pts[j].y;
                    p.z = 1;
        ...
        if (!init_pub) {
            ...
            pub_img->publish(std::move(feature_points));
